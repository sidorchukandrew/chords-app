...
export default function HighlightColorBottomSheetScreen() {
  const {surface, isDark} = useTheme();
  const {format, id} = useSelector(selectSongOnScreen);
  const [selectedColor, setSelectedColor] = useState(format.highlight_color);
  ...
  const debounce = useCallback(
    _.debounce(
      newColor => {
        let updatedFormat = {...format, highlight_color: newColor};
